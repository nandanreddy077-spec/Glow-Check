-- Additional Supabase Functions for Subscription Management
...
BEGIN
  ...
  -- If trial tracking doesn't exist, create it
  IF NOT FOUND THEN
  ...
  -- Create subscription record in pending state (will be activated after trial)
...
RETURNS VOID AS $$
BEGIN
  -- Increment analyses_used in trial_tracking
  UPDATE public.trial_tracking
  SET 
